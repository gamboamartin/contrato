<?php
namespace gamboamartin\contrato\src;
use Exception;
...
class rfc
{
    ...
    private $conjunciones = ['DA', 'DAS', 'DE', 'DEL', 'DER', 'DI', 'DIE', 'DD', 'EL', 'LA', 'LOS', 'LAS', 'LE', 'LES'
        , 'MAC', 'MC', 'VAN', 'VON', 'Y'];
    private $contracciones = ['MA', 'MA.', 'MARIA', 'JOSE', 'J', 'J.', 'MARÍA', 'JOSÉ'];
    ...
    public function calcular_rfc($nombre, $apellidoPaterno, $apellidoMaterno, $fecha)
    {
        ...
        $nombre = $this->quitar_articulos_nombre($nombre);
        $apellidoPaterno = $this->quitar_articulos($apellidoPaterno);
        $apellidoMaterno = $this->quitar_articulos($apellidoMaterno);
        ...
        $rfc = $this->remplazo_palabras($rfc);

        //La fecha debe ser con formato YYYY-MM-DD
        $rfc .= substr($fecha, 2, 2) . substr($fecha, 5, 2) . substr($fecha, 8, 2);

        //Le agregamos la homoclave al rfc
        $this->calcular_homoclave($nombrehomo, $fecha, $rfc);
        return $rfc;
    }

    private function calcular_homoclave($nombreCompleto, $fecha, &$rfc)
    {
    ...
    //esta funcion reemplaza palabras prohibidas
    private function remplazo_palabras($palabra)
    {
    ...
    private function quitar_articulos($palabra)
    {
    ...
    private function quitar_articulos_nombre($palabra)
    {
        $palabra = str_replace("DEL ", "", $palabra);
        $palabra = str_replace("LAS ", "", $palabra);
        $palabra = str_replace("DE ", "", $palabra);
    ...
    private function es_vocal($letra)
    {
        if (
